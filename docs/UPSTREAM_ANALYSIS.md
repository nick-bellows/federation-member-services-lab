...
- The public form's super-admin token means every anonymous applicant is, server-side, the most privileged principal in the system. `Server::handleClubAssociation` allows it to write to any club because the super admin passes `can('view', $club)`. A `club admin` token attempting the same call would find `$model->club` scoped to null and be silently re-associated to the admin's own club by the `getPermissionsTeamId()` branch rather than rejected.

...
1. **Separate release images, built from the same source.** `docker/api/api.release.Dockerfile` installs Composer dependencies without development packages at build time, builds the admin assets, removes the build toolchain, answers a `HEALTHCHECK` on liveness, and carries no environment file (since C2 the API container starts as the application user: nginx listens on 8080 and PHP-FPM on the loopback, so nothing needs root; before C2 it started as root to bind port 80 and dropped privileges, as upstream's image does, and the worker and scheduler ran as the user outright): a per-Dockerfile ignore list excludes every `.env*`, `vendor`, `node_modules`, the tests and the documentation. The web app keeps upstream's standalone Next.js image with its own ignore list, which excludes `.env.local` (the file that holds upstream's super-admin token in development). The development images are untouched.
...
## Addendum 2026-09-10: the release proof hardened (C2)

The external reviews of 2026-09-04 found that the proof's load-balancer rule `/api/*` would also catch NextAuth's callback at `/api/auth/callback/auth0` and send it to Laravel, so a sign-in through the proof would have died at the callback. C2 adds a rule for `/api/auth/*` to the web target group with a lower priority number, and a precondition on the API rule that fails the plan if the order ever drifts. Two further changes from the same review: the database storage is encrypted at rest (the proof still holds synthetic personal data), and the API release image is rootless (`USER verein`, nginx on 8080, PHP-FPM on the loopback, nginx's logs on the container's streams), with the Compose rehearsal and the Terraform port mappings following; the `terraform validate` record and the rehearsal record of 2026-09-10 are the evidence. `versions.tf` carries the remote-state note a shared apply needs. Nothing here changes the status: the proof remains validated, not applied.

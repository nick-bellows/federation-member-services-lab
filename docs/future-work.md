...
- **Pipeline hardening** — image scanning in CI, base images pinned by digest, actions pinned by SHA, a tag for upstream's Swagger UI image, the images pushed to a registry under the git SHA (the release checklist assumes a registry).

...
- **Advisories that need a major** (B7 audit, `docs/baseline/security_audit_2026-09-03.txt`) — `next` 14.2 (four advisories; fix is Next 16), `sharp` 0.33 (fix 0.35), `swiper` 9 (critical prototype pollution; fix 14); none used by the federation pages beyond `next` itself. Re-evaluated at each B8 release per the policy in `docs/THREAT_MODEL.md`; the within-major fixes (Filament 5.7.6, commonmark 2.10, Livewire, `nanoid`, `js-yaml`, `brace-expansion`) are B8's first release step.

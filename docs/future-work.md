...
- **Infrastructure as code** — `deploy/terraform/` holds the minimal proof of `docs/DEPLOYMENT.md` (validated, not applied; ADR-0015 addendum). The first `plan` and `apply` against the owner's account are the evidence that turns it validated in fact; the production shape (private subnets, NAT or endpoints, Multi-AZ, WAF, a domain and certificate) stays a follow-up.
...
- **A hosted case study page** — approved 2026-09-04: `docs/index.md` and `docs/_config.yml` make `docs/` a GitHub Pages site; enabled on the fork once merged.
- **A hosted demo** — the recording in `docs/assets/demo.webm` stands in; a running demo is the Terraform proof in `deploy/terraform/` once the owner's AWS credentials exist (approved 2026-09-04, not applied yet).

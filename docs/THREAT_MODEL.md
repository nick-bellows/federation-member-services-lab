...
Status: reviewed 2026-09-03 (B7); leaves updated 2026-09-10 after the external reviews (C1, ADR-0016). Method: attack trees (owner's decision, ADR-0014), one tree per attacker goal, STRIDE used as the checklist when listing a node's children. Every leaf names the control in the code that stops it, or the gap and where the gap is recorded. Nothing here is a claim about production: the system is validated in CI and runs in Compose, and the controls are the ones the tests exercise.
...
- 1.2 Send a JSON Patch (B7) with an operation on a field I may not touch
  ...
  - 1.2.5 overwrite a concurrent change → **Partly.** A `test` operation lets the client guard its view (409 `patch_test_failed`); the row is locked for the transaction and, since C1, the allow-list is computed on the locked row. Residual: a client that sends no `test` writes last-wins, which is the JSON:API update's behaviour too.
  ...
  - 1.2.7 race a patch, a document attachment or a document review against a transition, so the check sees a draft and the write lands on a submitted or decided application → **Mitigated (C1).** The three actions lock the row first and decide on what they locked (ADR-0016); the interleaving is simulated in `ConcurrentAuthorizationTest`, one test per action. Before C1 the checks ran on the row the request had loaded, which the reviews of 2026-09-04 found.
- 1.3 Perform a transition I may not perform
  ...
  - 1.3.4 present another person's idempotency key to receive their application, or my own key with another window or role to be answered as if nothing changed, or a replay that rewrites the details → **Mitigated (C1).** A key belongs to the applicant who presented it (`(applicant_user_id, idempotency_key)` unique); the same key with other parameters is 409 `idempotency_key_reused`; a replay answers the stored application and never rewrites it (`StartApplicationTest`, `RegistrationApplicationsHttpTest::test_a_replayed_start_answers_the_stored_application_and_never_rewrites_it`). Before C1 the key was looked up globally and the controller re-filled the details on a replay.
...
- 1.6 Open a registration window on another federation's season, so applications land in a season the organization's federation does not run → **Mitigated (C1).** The controller checks the season's federation against the organization's and answers 409 `season_not_in_federation` for organization and federation administrators alike, with no row and no audit entry (`RegistrationWindowsHttpTest::test_a_window_cannot_be_opened_on_another_federations_season`). Before C1 the exception existed and was thrown nowhere.
...
- 3.3 Obtain the federation's service token → **Partly.** The client secret lives in `.env` (gitignored; since C3 the `secret-scan` job runs gitleaks over the whole history on every push and pull request and fails the build on a finding, with the one inherited development token listed by fingerprint in `.gitleaksignore`); the token is cached in Laravel's cache store for its lifetime minus a margin. Residual: the cache store is the file store in Compose, readable by anyone with container access; the production store is B8's decision (`docs/future-work.md`).
...
- 4.8 Make the provider answer about another person (a routing or caching fault on its side, or a compromised provider), so one person's eligibility is stored as another's → **Mitigated (C1).** The answer is bound to the subject that was asked for at the value object (`CredentialFacts::fromArray`), refused as a contract mismatch, never stored: 502 `learning_center_error` on a reviewer's refresh, a parked event after an approval; the stored snapshot is bound again on read (`CredentialFactsTest`, `CredentialSnapshotsTest`, `ParticipationHttpTest`). Before C1 nothing compared the two, and a test normalised the mismatch.
...
- 6.6 CI → **Partly.** Actions are pinned by major tag; the workflow runs on the fork's own runners with no secrets beyond `GITHUB_TOKEN`; pull requests from forks of the fork would run the same workflow (no secrets to leak). Upstream's `publish.yml` interpolated the merged pull request's branch name into a `run:` script; since C1 it reaches the shell as an environment variable (offered upstream as item 5). Residual: SHA pinning of actions (B8).

...
| Upstream baseline and now | Upstream: PHPUnit 91/91 (338 assertions, 58 s, SQLite); Pint 215 issues; `tsc` 0 errors; ESLint 71 warnings; no frontend or E2E tests; no CI that runs tests. Fork at `a68f88f`: PHPUnit 237 (1,096 assertions on SQLite, 1,095 on MariaDB and PostgreSQL); the workflow runs eight jobs; CI history: the first two runs of PR #1 failed, one run on the B4 branch failed on 2026-09-03 (a stalled MariaDB runner, cancelled and re-run), every merge to `main` since 2026-09-03 has been green |
...
| --- | --- | --- | --- | --- |
| C1 Domain and security defects | Scope the start idempotency key to the applicant and refuse a reused key with different parameters (409); never re-apply details on a replay. Bind the Learning Center response to the requested subject and reject a mismatch as a contract error (the test that normalised it is corrected). Enforce that a window's season belongs to the organization's federation (the unused exception gets used; 409 `season_not_in_federation`). Recompute authorization on the locked row in the patch and the two document actions. Pass the branch name through an environment variable in upstream's publish workflow. | Regression tests for each (cross-user key, parameter mismatch, wrong subject, cross-federation season); ADR-0016 on the boundary rules; `docs/THREAT_MODEL.md` leaves updated from partly to mitigated; the workflow fix added as item 5 of the upstream offer draft | none | 1 day |
...
| O6 The upstream offer, last | Say "send it" | Open the one issue from `docs/UPSTREAM_OFFER.md` (five items once C1 adds the workflow fix), then record the maintainers' answer truthfully in the README and this file, whatever it is; re-cut only what they ask for onto upstream `main` | C1 to C5 merged; O4 done |

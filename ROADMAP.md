...
Last verified: 2026-09-10 (C1 done; Phase C was added 2026-09-04 after two external reviews of `a68f88f`). Supersedes the 2026-09-02 and 2026-09-03 versions. Governed by the workspace `AGENTS.md` and the central portfolio roadmap; where they conflict, the central roadmap wins.

## Handoff snapshot

| Field | Current state |
| --- | --- |
| Lifecycle | `VALIDATED` in CI, not deployed — Phase A complete: M0 to M4 (archaeology, baseline fix, domain and state machine, OIDC identity, registration-review slice with browser tests), README, cold clone, public fork with CI green on `main` (2026-09-03). Phase B: B2 Learning Center contract (ADR-0009, INCIDENT-001) and B3 events and reliability (ADR-0010, INCIDENT-003) and B4 PostgreSQL (ADR-0011, compatibility matrix) and B5 operability (ADR-0012, runbook) and B6 accessibility and performance (ADR-0013) and B7 security review (ADR-0014, `docs/THREAT_MODEL.md`) done 2026-09-03; B8 release engineering (ADR-0015, design only, release rehearsal) and B9 case study and demo done 2026-09-04 for everything that does not need the owner. **Owner's answers 2026-09-04:** provisioning approved (Terraform proof written and validated; waits for AWS credentials), GitHub Pages approved (live at https://nick-bellows.github.io/federation-member-services-lab/), the Auth0 tenant to be created by the owner (walkthrough ready), the upstream offer moved to the last item and still unsent (decision 7). **Phase C planned 2026-09-04 from the external reviews: C1 to C5 by Claude Code, O1 to O6 by the owner. C1 done 2026-09-10 (ADR-0016; ten regression tests retained failing in `docs/baseline/c1_tests_before.txt` and passing in `c1_tests_after.txt`; the flaky upstream consent test that failed the #14 merge fixed; offer items 5 and 6 added); next: C2** |
| Branch | `main` of the fork, merged through pull requests (#1 Phase A, #2 to #4 docs, #5 B2, #6 B3, #7 B4, #8 B5, #9 B6, #10 B7, #11 B8, #12 B9, #13 Pages and Terraform, #14 Phase C plan, #15 C1); 61 commits ahead of upstream `main` (`dca9be3`) at `e5e15c1`, before C1 |
...
| Upstream baseline and now | Upstream: PHPUnit 91/91 (338 assertions, 58 s, SQLite); Pint 215 issues; `tsc` 0 errors; ESLint 71 warnings; no frontend or E2E tests; no CI that runs tests. Fork after C1: PHPUnit 247 (1,148 assertions on SQLite, `docs/baseline/phpunit_after_c1_backend.txt`; the servers run in CI); the workflow runs eight jobs; CI history: the first two runs of PR #1 failed, one run on the B4 branch failed on 2026-09-03 (a stalled MariaDB runner, cancelled and re-run), the merge of #14 failed on 2026-09-05 on the MariaDB job (upstream's consent test compared two `now()` reads; fixed in C1), every other merge to `main` since 2026-09-03 has been green |
...
| C2 Release proof hardening | A higher-priority load-balancer rule sends the identity callback path to the web app, with a Terraform precondition so the two rules cannot drift; database storage encrypted; the API release image runs nginx and PHP-FPM as the application user on an unprivileged port, with the compose rehearsal and the Terraform port mappings following; a note on remote state before any apply | `terraform validate` re-recorded; the release rehearsal re-run and re-recorded with the rootless image | C1 merged | half a day |
| C3 Secret-scan gate | A gitleaks job in CI that fails the build, with an allowlist for the one inherited development seed credential upstream committed | the job green on its first pull request; the allowlist reviewed in the pull request | none | 1 hour |
| C4 Evidence and reviewer path | README: 237 tests, the browser count as CI runs it (9 passed, 3 skipped by design), 16 decision records, the workflow no longer "draft", the real CI history, the cold-clone date, a link to the Pages site, "the application is not deployed; the documentation site is". ROADMAP and the case study aligned. Pages serves the baseline folder; the case study's evidence names become links pinned to a commit. Screenshots recaptured with waits and a populated queue; the demo embedded on the landing page with a poster; Pages link and footer styling fixed for the two axe failures. `docs/future-work.md` reduced to what is still open, duplicates removed. | the Pages site re-checked page by page; axe clean on the landing and case-study pages; the README read against the numbers in `docs/baseline/` | C1 to C3 merged (so the numbers are final) | half a day |
| C5 Final verification | Three-engine suite and browser journeys in CI on the closing merge; the release rehearsal; a cold clone of the README path once more; this file's snapshot, the learning log and the interview guide brought to the closing state; the internal record completed for Phase C | `docs/baseline/phpunit_after_c_backend.txt`, `release_rehearsal_<date>.txt`, `cold_clone_<date>.txt`; CI green on `main` | C4 merged | half a day |

### O1 to O6: wait for the owner

| Item | What the owner does | What Claude Code does once it exists | Gate |
| --- | --- | --- | --- |
| O1 Auth0 tenant | Create the tenant, API, application and two test users per `docs/AUTH0_WALKTHROUGH.md` steps 1 to 5; put the tenant domain, client id and client secret into `web_application/.env.local` and `api/.env` (both gitignored); say when done | Run the walk, take the six screenshots into `docs/assets/auth0/`, turn the walkthrough from planned to validated, add the CloudFront callback URL to the tenant notes once O2 exists | none |
| O2 AWS proof | An AWS account with an administrator user or role for the proof, configured as a profile under `~/.aws` on this machine; the profile name and region; a go on the cost (about $2.50 a day while running, `deploy/terraform/README.md`) and on the teardown date | `terraform plan` and `apply` through Docker, both recorded under `docs/baseline/`; images pushed under the git SHA; the migration task and the seed; the release checklist walked against the public URL; `destroy` on the owner's word, recorded | C2 merged; O1 for the sign-in step |
| O3 Fork releases | Decide whether the fork carries GitHub releases at all. If yes: dispatch upstream's `release.yml`, review the `release/<version>` pull request, merge it, and `publish.yml` tags and publishes | Nothing until decided; then the release notes and the tag recorded in `docs/RELEASE.md` | C1 (the workflow fix) merged |
| O4 The owner's read | The README's one-minute and five-minute reads as a stranger, with the timer; the internal quiz review (lessons, quizzes, exercises, interview questions and the sealed grader notes for M0 to B9 and Phase C) | Fix whatever the read finds | C4 merged |
| O5 Profile placement | Whether and when to pin the repository on the GitHub profile; the central roadmap keeps it unpinned until the upstream offer has an answer | Nothing; a pin is an account-level change the owner makes | O6 answered |
| O6 The upstream offer, last | Say "send it" | Open the one issue from `docs/UPSTREAM_OFFER.md` (six items: the four from B9, the workflow fix and the flaky consent test from C1), then record the maintainers' answer truthfully in the README and this file, whatever it is; re-cut only what they ask for onto upstream `main` | C1 to C5 merged; O4 done |
...
9. **After B9 (2026-09-04): the upstream offer moves to the last item; the Auth0 tenant, provisioning and a hosted case study proceed.** Provisioning became the Terraform proof (ADR-0015 addendum), the case study became the GitHub Pages site; the tenant and the AWS credentials are the owner's inputs (O1, O2).
10. **External reviews adopted as Phase C (2026-09-04).** Two independent reviews of `a68f88f` were verified finding by finding against the code; the confirmed defects and drift became C1 to C5, the owner-held items O1 to O6. The data-engineering calibration of one review is recorded as the wrong bar for this repository and not adopted. Loopback binding of the Compose ports is tried in C2 and adopted only if the mock identity provider still works on Windows.

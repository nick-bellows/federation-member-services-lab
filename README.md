...
Not done, by design: no deployment exists (the architecture is designed and priced at nothing); nothing has been offered upstream (one offer is drafted and waits for the owner's word); the Auth0 walkthrough waits for a tenant. The order and gates are in [`ROADMAP.md`](ROADMAP.md).
